...
static const char *TAG = "BMI2_ESP32";
...
BMI2_INTF_RETURN_TYPE bmi2_i2c_read(uint8_t reg_addr, uint8_t *reg_data, uint32_t len, void *intf_ptr)
{
    ...
    esp_err_t ret = i2c_bus_read_bytes(intf_info, reg_addr, (uint16_t)len, reg_data);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C read reg 0x%02X len %u failed: %s", reg_addr, (unsigned)len, esp_err_to_name(ret));
...
{
    i2c_bus_device_handle_t intf_info = (i2c_bus_device_handle_t)intf_ptr;
    esp_err_t ret = i2c_bus_write_bytes(intf_info, reg_addr, (uint16_t)len, reg_data);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "I2C write reg 0x%02X len %u failed: %s", reg_addr, (unsigned)len, esp_err_to_name(ret));
        return BMI2_E_COM_FAIL;
    }
    return BMI2_INTF_RET_SUCCESS;
}
...
int8_t bmi2_interface_init(struct bmi2_dev *bmi, uint8_t intf, uint8_t dev_addr, i2c_bus_handle_t bus_inst)
{
    ...
    if (bmi != NULL) {
        ...
        if (BMI2_OK == result) {
            ...
            ESP_LOGI(TAG, "Creating I2C device with address 0x%02X", dev_addr);
