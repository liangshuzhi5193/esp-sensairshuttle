...
static bmi270_circle_handle_t bmi_handle = NULL;
...
static int8_t set_feature_config(struct bmi2_dev *bmi2_dev)
{
    ...
    config[0].type = BMI2_CIRCLE_GESTURE_DETECTION;
    ...
    if (rslt == BMI2_OK)
    {
        ...
        // 0x01-> X Axis 0x02 -> Y Axis 0x03-> Z Axis
        config[0].cfg.circle_gest_det.cgd_cfg1.axis_of_rotation = 0x03; // Set axis of rotation
 ...
 * @return Result code from BMI2 API
 */
static int8_t bmi270_enable_circle_gesture_int(struct bmi2_dev *bmi2_dev)
{
    int8_t rslt;
    uint8_t sens_list[2] = { BMI2_ACCEL, BMI2_CIRCLE_GESTURE_DETECTION };
    uint16_t int_status = 0;
    uint8_t circle_output;
    uint8_t direction;
    struct bmi2_sens_int_config sens_int = { 
        .type = BMI2_CIRCLE_GESTURE_DETECTION, 
            ...
            while (rslt == BMI2_OK) {
                if (interrupt_status) {
                    ...
                    if (int_status & BMI270_CIRCLE_CIRCLE_GESTURE_MASK)
                    {
                        ...
                        direction = circle_output & 0x07;
                        circle_output = circle_output & 0x08;
