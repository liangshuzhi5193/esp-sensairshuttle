...
static int8_t set_accel_gyro_config(struct bmi2_dev *bmi2_dev)
{
    ...
    if (rslt == BMI2_OK) {
        /* Set Output Data Rate - Increase sampling rate for better response */
        config[BMI2_ACCEL].cfg.acc.odr = BMI2_ACC_ODR_400HZ;
        config[BMI2_ACCEL].cfg.acc.range = BMI2_ACC_RANGE_8G;
        config[BMI2_ACCEL].cfg.acc.bwp = BMI2_ACC_NORMAL_AVG4;
        config[BMI2_ACCEL].cfg.acc.filter_perf = BMI2_PERF_OPT_MODE;

        config[BMI2_GYRO].cfg.gyr.odr = BMI2_GYR_ODR_400HZ;
        config[BMI2_GYRO].cfg.gyr.range = BMI2_GYR_RANGE_1000;
        ...
        /* Interrupt pin configuration for INT1 (Push) */
...
static int8_t bmi270_toy_enable_push_int(struct bmi2_dev *bmi2_dev)
{
    ...
    // enable features (following official example)
...
void app_main(void) {
    ...
    // Configure GPIO for interrupt (INT1 for push detection)
    gpio_config_t io_conf = {
        .intr_type = GPIO_INTR_POSEDGE,  // Active high for INT1
