...
static int8_t set_feature_interrupt(struct bmi2_dev *bmi2_dev)
{
    ...
    // Map toy-motion interrupt to INT1 (following toy_demo.c style)
...
static void adjust_toy_motion_config(struct bmi2_dev *bmi2_dev)
{
    uint8_t data[2], page;
    int8_t rslt;
...
static int8_t bmi270_toy_enable_toy_motion_int(struct bmi2_dev *bmi2_dev)
{
    int8_t rslt;
    ...
    rslt = enable_toy_motion(bmi2_dev, BMI2_ENABLE);
    bmi2_error_codes_print_result(rslt);
