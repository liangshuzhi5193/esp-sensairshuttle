...
static int8_t set_feature_interrupt(struct bmi2_dev *bmi2_dev)
{
    ...
    // Map any-motion interrupt to INT1 (following toy_demo.c style)
...
static int8_t bmi270_toy_enable_any_motion_int(struct bmi2_dev *bmi2_dev)
{
    int8_t rslt;
    ...
    rslt = enable_toy_any_motion(bmi2_dev, BMI2_ENABLE);
    bmi2_error_codes_print_result(rslt);
        ...
        while (rslt == BMI2_OK) {
            if (interrupt_status) {
                ...
                if (int_status & BMI270_TOY_INT_ANY_MOT_MASK) {
                    ESP_LOGI("MAIN", "Any-motion interrupt generated! Status: 0x%02X", int_status);
                }
            } else {
