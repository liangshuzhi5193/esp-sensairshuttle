...
static int8_t set_feature_interrupt(struct bmi2_dev *bmi2_dev)
{
    ...
    // Map shake interrupt to INT1 (following toy_demo.c style)
...
static int8_t bmi270_toy_enable_shake_int(struct bmi2_dev *bmi2_dev)
{
    int8_t rslt;
    ...
    rslt = enable_toy_shake(bmi2_dev, BMI2_ENABLE);
    bmi2_error_codes_print_result(rslt);
        ...
        while (rslt == BMI2_OK) {
            if (interrupt_status) {
                ...
                if (int_status & BMI270_TOY_INT_SHAKE_MASK) {
                    ...
                    // slight shake
                    if(data & 0x08) {
                        ESP_LOGI("MAIN", "Slight shake generated on: ");
                        if(data & 0x01) {
                    ...
                    if(data & 0x80) {
                        ESP_LOGI("MAIN", "Heavy shake generated on: ");
                        if(data & 0x10) {
                            ESP_LOGI("MAIN", "x axis");
                        }
                        if(data & 0x20) {
...
void app_main(void) {
    ...
    ESP_LOGI("MAIN", "=== BMI270 TOY Shake Detection Configuration ===");
    ...
    int8_t rslt = bmi270_toy_enable_shake_int(bmi_handle);
    bmi2_error_codes_print_result(rslt);
    if (rslt != BMI2_OK) {
        ESP_LOGE("MAIN", "BMI270 TOY enable shake interrupt failed");
    }
    ...
    ESP_LOGI("MAIN", "BMI270 TOY shake interrupt example finished.");
}
